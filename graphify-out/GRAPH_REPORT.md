...
## Corpus Check
- 132 files · ~117,920 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1416 nodes · 2995 edges · 157 communities (134 shown, 23 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 2 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7868e89a`
...
8. `NotebookEditor()` - 23 edges
9. `GhostButton()` - 21 edges
10. `formatHumanDate()` - 21 edges

## Surprising Connections (you probably didn't know these)
- `CalendarCell()` --calls--> `getTodayDateString()`  [EXTRACTED]
  src/screens/LibraryScreen.tsx → src/lib/date.ts
- `findTodayPlan()` --calls--> `getTodayDateString()`  [EXTRACTED]
  src/store/useMonkStore.ts → src/lib/date.ts
- `App()` --calls--> `startEveningNudgeWatcher()`  [EXTRACTED]
  src/app/App.tsx → src/lib/eveningNudge.ts
- `App()` --calls--> `useMonkStore`  [EXTRACTED]
...
### Community 0 - "App.tsx"
Cohesion: 0.10
Nodes (39): ArchiveScreenLazy, FocusScreen, JournalEntryScreenLazy, JournalLibraryScreenLazy, LearningScreenLazy, NotebookPageLazy, PacksPageLazy, RelapseScreenLazy (+31 more)

### Community 1 - "useMonkStore"
Cohesion: 0.06
Nodes (86): OnboardingGate(), OnboardingScreen(), ProtectedMain(), RootRedirect(), DefenseChips(), DrawingCanvasProps, Stroke, LoginScreen() (+78 more)

### Community 2 - "focusMusic.ts"
Cohesion: 0.10
Nodes (27): AFTERNOON, buildGraph(), clearGraph(), createNoiseBuffer(), dayOfYear(), ensureRunning(), EVENING, extraNodes (+19 more)
...
Cohesion: 0.16
Nodes (7): createInitialState(), mkState(), baseState(), today, baseState(), today, MonkMVPState
...
### Community 9 - "app.ts"
Cohesion: 0.09
Nodes (22): BadHabit, DateOnlyString, DayStatus, DayType, GoalDraft, GoalStatus, ISODateString, JournalEntry (+14 more)
...
Cohesion: 0.11
Nodes (19): date-fns, lucide-react, dependencies, date-fns, lucide-react, react, react-dom, react-router-dom (+11 more)
...
### Community 16 - "TodayScreen.tsx"
Cohesion: 0.15
Nodes (30): WhyCard(), FOCUS_PRESETS, CORE_VALUES, dismissReentry(), dismissReflectionThread(), getDayPart(), hideReentryChip(), isCloseDaySkipped() (+22 more)

### Community 17 - "FocusSession.tsx"
Cohesion: 0.18
Nodes (23): App(), getCompletedSeconds(), getCurrentFocusPhase(), getSharedCtx(), playZenBell(), unlockAudio(), isMusicOn(), toggleMusic() (+15 more)

### Community 18 - "selectors.ts"
Cohesion: 0.14
Nodes (16): fireEveningNudge(), isCloseDaySkipped(), markNudged(), shouldEveningNudge(), startEveningNudgeWatcher(), wasNudgedToday(), WeeklyStatusIndicators(), WeekScreen() (+8 more)

### Community 19 - "planScoring.ts"
Cohesion: 0.22
Nodes (12): defaultWeeklyTargets(), capacityCheck(), PlanScore, planStrengthLabel(), scoreAntiGoals(), scoreDuration(), scoreKeystoneActions(), scorePlan() (+4 more)
...
Cohesion: 0.13
Nodes (17): BUILT_IN_JOURNAL_PACKS, createDefaultOnboarding(), createDefaultSettings(), DEFAULT_NOTEBOOK_CATEGORIES, frictionActionsForHabit(), habitOptions, learningTypes, nowIso() (+9 more)
...
Cohesion: 0.20
Nodes (10): 9.2 Card, 9.3 Input, 9.4 Textarea, 9.5 Chip, 9.6 Bottom Navigation, 9. Component System, Default Card, Focus Card (+2 more)

### Community 33 - "date.ts"
Cohesion: 0.38
Nodes (10): SeasonProgressCard(), getCurrentWeekNumber(), getDayNumber(), getDaysLeft(), getDaysPassed(), getSeasonProgress(), isDatePast(), isSeasonEnded() (+2 more)
...
Cohesion: 0.11
Nodes (33): InlinePhoto(), LightboxImage(), photoIdsInBody(), PhotoLightbox(), PhotoOpenHandler, useObjectUrl(), compressImage(), deleteImage() (+25 more)
...
Cohesion: 0.12
Nodes (27): ExpectedFocusSessionConfig, FocusSessionRecord, formatFocusSessionTimelineDescription(), formatMinutes(), formatTotal(), getExpectedSessionConfig(), getFocusSessionPreset(), getFocusSessionTiming() (+19 more)
...
### Community 134 - "parseIntention"
Cohesion: 0.31
Nodes (7): formatIntention(), isStructuredIntention(), parseIntention(), formatted, parsed, KeystoneSetup(), ObstacleStep()

### Community 150 - "dailyActivityStatus.ts"
Cohesion: 0.28
Nodes (7): DAILY_STATUS_LABELS, DailyActivity, DailyActivityStatus, getDailyStatusHelper(), resolveDailyActivityStatus(), LearningSession, TimelineStatus

### Community 151 - "focusPresets.ts"
Cohesion: 0.32
Nodes (6): createFocusPhases(), FocusPresetConfig, getTotalPlannedMinutes(), summarizeFocusSession(), FocusSessionPhase, FocusSessionTimelineDetails

### Community 152 - "9.1 Button"
Cohesion: 0.50
Nodes (4): 9.1 Button, Ghost Button, Primary Button, Secondary Button
...
- **Why does `useMonkStore` connect `useMonkStore` to `App.tsx`, `date.ts`, `parseIntention`, `MonkMVPState`, `useMonkStore.ts`, `JournalNotebook.tsx`, `TodayScreen.tsx`, `FocusSession.tsx`, `selectors.ts`, `planScoring.ts`, `focusSessionStatus.ts`, `useMonkStore.notebook.test.ts`, `i18n/index.ts`, `SettingsScreen.tsx`, `main.tsx`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `useT()` connect `useMonkStore` to `App.tsx`, `JournalNotebook.tsx`, `TodayScreen.tsx`, `FocusSession.tsx`, `focusSessionStatus.ts`, `selectors.ts`, `PwaInstallBanner.tsx`, `i18n/index.ts`, `SettingsScreen.tsx`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **Why does `t()` connect `useMonkStore` to `App.tsx`, `useMonkStore.ts`, `JournalNotebook.tsx`, `TodayScreen.tsx`, `FocusSession.tsx`, `selectors.ts`, `focusSessionStatus.ts`, `PwaInstallBanner.tsx`, `i18n/index.ts`?**
...
- **Should `useMonkStore` be split into smaller, more focused modules?**
  _Cohesion score 0.059496567505720827 - nodes in this community are weakly interconnected._
- **Should `focusMusic.ts` be split into smaller, more focused modules?**
